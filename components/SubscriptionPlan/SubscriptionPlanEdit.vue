...
onMounted(async () => {
  ...
  const plan = store.response_detail_query_data;
  if (plan) {
    request.value.plan_name = plan.plan_name;
    request.value.price_monthly = plan.price_monthly;
    request.value.max_users = plan.max_users;
    request.value.max_products = plan.max_products;
    request.value.features = plan.features;
  }
});
...
<template>
  <section class="pa-6">
    <v-card elevation="0" class="pa-6">
      <GlobalTextTitleLine title="ແກ້ໄຂແພັກເກັດຄ່າບໍລິການ" class="mb-8">
        <template v-if="permission.can_update" #actions>
          <v-btn color="primary" flat type="submit" form="subscription-plan-edit-form" :loading="loading"
            >ບັນທຶກ</v-btn
          >
        </template>
      </GlobalTextTitleLine>
      ...
      <v-form v-else id="subscription-plan-edit-form" ref="form" @submit.prevent="submitForm">
        <v-row>
          <v-col cols="12" md="6">
