...
const headers = ref([
  ...
  { title: "ລະຫັດ", key: "code", sortable: false },
  { title: "ຮ້ານຄ້າ", key: "shop_id", sortable: false },
  { title: "ປະເພດ", key: "discount_type", sortable: false },
  { title: "ມູນຄ່າ", key: "discount_value", sortable: false },
  { title: "ໃຊ້ແລ້ວ/ຈຳກັດ", key: "used_count", sortable: false },
  ...
  <div class="pa-6">
    <v-card elevation="0" tile width="100%" min-height="95vh" class="pa-6">
      <v-row>
        <v-col cols="12">
          <GlobalTextTitleLine
            :title="`ຈັດການສ່ວນຫຼຸດ / Manage Discount (${formatNumber(
        ...
        <v-col cols="12" class="d-flex flex-wrap justify-space-between align-center">
          ...
          <div v-if="permission.can_create" class="d-flex flex-wrap align-center">
            <v-btn color="primary" elevation="0" @click="goPath('/discount/create')">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              <v-btn
                ...
                icon="mdi-pencil"
                variant="text"
                @click="goPath(`/discount/edit?id=${item.id}`)"
                size="small"
              ></v-btn>

              <v-btn
                v-if="permission.can_delete && item.is_active"
                color="error"
                icon="mdi-delete"
                variant="text"
